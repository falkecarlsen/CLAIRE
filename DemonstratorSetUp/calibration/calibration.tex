...
\begin{frame}{Experiment design}
	\small
	\begin{itemize}
		\item Script \texttt{calibration\_static\_outflow.py}.
		\item All pumps are off.
		\item Water level measured as quickly as possible (i.e., delay is coming from the setup, not the Python script).
		\item Total of 200 measurements.
	\end{itemize}
\end{frame}

\begin{frame}{Experimental results - static outflow July 25}
	\centering
	\begin{tikzpicture}
		% To plot figure with multiple ordinates, see section 4.9.11 of the pgfplot manual.
		pgfplotsset{set layers}
...
%			mark=x,
			...
			table[x index = 0,y index = 1, col sep=comma]{data/calibration_outflow_data.txt};
			\addlegendentry{Tube 1}
...
%			mark=x,
			...
			table[x index = 0,y index = 2, col sep=comma]{data/calibration_outflow_data.txt};
			\addlegendentry{Tube 2}
		\end{axis}
	\end{tikzpicture}
\end{frame}

\begin{frame}{Experimental results - static outflow August 28}
	\centering
	\begin{tikzpicture}
		% To plot figure with multiple ordinates, see section 4.9.11 of the pgfplot manual.
		pgfplotsset{set layers}
		\begin{axis}[
			...
			table[x index = 0,y expr=900 - \thisrowno{1}, col sep=comma]{data/calibration_outflow_data_2024-08-28.txt};
			\addlegendentry{Tube 1}
			...
			table[x index = 0,y expr=900 - \thisrowno{2}, col sep=comma]{data/calibration_outflow_data_2024-08-28.txt};
			\addlegendentry{Tube 2}
		\end{axis}
	\end{tikzpicture}
\end{frame}
