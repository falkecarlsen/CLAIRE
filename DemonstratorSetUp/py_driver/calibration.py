...
def calibration_tube(tube, duty_level, file):
    ...
    """
    claire.state.make_dynamic()  # Start each calibration of a tube with 'outdated' data.
    for i in range(5):
...
def calibration_inflow(tube, duty_level, file):
    ...
    :param file: The file to write the results to.
    """
    calibration_flow(tube, 100, 500, duty_level, file)
...
def calibration_outflow(tube, duty_level, file):
    ...
    :param file: The file to write the results to.
    """
    calibration_flow(tube, 500, 100, duty_level, file)
...
def calibration_flow(tube, start_level, end_level, duty_level, file):
    ...
    if measure_inflow and state.get_tube_level(tube) >= start_level:
        claire.set_water_level(tube, start_level)
    if not measure_inflow and state.get_tube_level(tube) <= start_level:
        claire.set_water_level(tube, start_level)
    ...
    state = claire.update_state()
    start_time = time()
    true_start_level = state.get_tube_level(tube)
    ...
    while True:
        try:
            state = claire.update_state()  # Requesting current state takes time, so no need to delay explicitly.
        except driver.SensorError:
            print("Sensor error, trying again.")
            continue

        # Check whether calibration can be finished.
        if measure_inflow and state.get_tube_level(tube) > end_level:
            break
        elif not measure_inflow and state.get_tube_level(tube) < end_level:
            break
    ...
    end_time = time()
    state = claire.update_state()
    true_end_level = state.get_tube_level(tube)
